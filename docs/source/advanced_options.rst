...
flag is used. To treat elastic networks in MartiniGlass, two arguments are necessary:

* ``-el`` specifies that there are proteins with elastic networks in the system.
* ``-ef`` specifies the force constant used in the elastic network that was used when generating the coarse grained topology.

For example:

.. code-block::

    $ martiniglass -p topol.top -el -ef 500

will process the system described in ``topol.top``. For any protein topologies in the system,
any bonds with the force constant of 500 kJ/mol/nm^2 (as specified by ``-ef``) will be identified
as the elastic network and written into the second set of topology files.
